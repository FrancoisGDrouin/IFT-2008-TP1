 ...
 */
void Labyrinthe::chargeLabyrinthe(Couleur couleur, std::ifstream &entree) //1
{
	int nbCols, nbRangs;
 ...
 * \param[in]	int j2
 */
void Labyrinthe::ajoutePassage(Couleur couleur, int i1, int j1, int i2, int j2) //3
{
 ...
 */
    void Labyrinthe::ajoutePieceLabyrinthe(const Piece& p) { //2
...
*/
    int Labyrinthe::solutionner(Couleur joueur) //4
    {
        std::queue<Piece *> file;
        setAttributParcourueAFalse();
        ...
        do
        {
            ...
            // 2.2 Enfiler toutes les pièces qui sont accessibles à partir de cette pièce à l'aide d'une porte
            ...
            // 2.2.1 Déterminer si une pièce connectée à la pièce courante contient une porte pouvant s'y rendre
            // de la bonne couleur
            chercherPorteBonneCouleurVersPieceCourante(portes, joueur, file, distance_courante);
            permetPassageVersPieceCourante(file, courant);
...
/**
  * \fn void Labyrinthe::chercherPorteBonneCouleurVersPieceCourante(const std::list<Porte> &portes, Couleur joueur, std::queue<Piece *> &file, int distance_courante)
...
*/
    void Labyrinthe::chercherPorteBonneCouleurVersPieceCourante(const std::list<Porte> &portes, Couleur joueur, std::queue<Piece *> &file, int distance_courante)
    {
...
/**
  * \fn void Labyrinthe::permetPassageVersPieceCourante(std::queue<Piece *> &file, Piece *pieceCourante)
...
*/
    void Labyrinthe::permetPassageVersPieceCourante(std::queue<Piece *> &file, Piece *pieceCourante)
    {
...
/**
  * \fn void Labyrinthe::setAttributParcourueAFalse()
  * \brief Méthode mettant à false l'attribut parcourue de toutes les pieces du labyrinthes
*/
    void Labyrinthe::setAttributParcourueAFalse()
    {
...
*/
    Couleur Labyrinthe::chercheMeilleurScore(const std::map<Couleur, int> &joueurs)
    {
        ...
        {
            return gagnantAmbigue(min.first);
        }
    }
/**
  * \fn Couleur Labyrinthe::gagnantAmbigue(const std::vector<Couleur> &joueurs) const
...
*/
    Couleur Labyrinthe::gagnantAmbigue(const std::vector<Couleur> &joueurs) const
    {
