...
private:
    //FGD D
    void setAttributParcourueAFalse();
    void chercherPorteBonneCouleurVersPieceCourante(const std::list<Porte> &portes, Couleur joueur, std::queue<Piece *> &file, int distance_courante);
    void permetPassageVersPieceCourante(std::queue<Piece *> &file, Piece *Courante);
    ...
    void _detruire();
    Couleur gagnantAmbigue(const std::vector<Couleur> &joueurs) const;
    //FGD F
    class NoeudListePieces //noeud typique d'une liste chaînée circulaire
    {
        public:
            ...
            // Le noeud suivant
            NoeudListePieces *suivant = nullptr; // noeud suivant
    };
	...
	//FGD D
	/*! Accesseur pour le noeud d'arrivée */
